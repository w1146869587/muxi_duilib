...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
	...
	HMODULE hmod = LoadLibrary(_T("ydftedit.dll"));
	if (hmod)
	{
...
BOOL CTxtWinHost::TxSetScrollPos (INT fnBar, INT nPos, BOOL fRedraw)
{
	...
	m_re->GetManager()->SendNotify(m_re, DUI_MSGTYPE_RICHEDIT_SCROLL_POSCHANGED);
...
void CTxtWinHost::SetClientRect(RECT *prc) 
{
	...
	//解决：最小化窗口，恢复后，滚动条滑块跑到对齐图片顶部；双击最大化窗口，history下面有一片空白，TXTBIT_CLIENTRECTCHANGE内部会触发UpdateView，
...
bool CRichEditUI::AppendText(LPCTSTR lpstrText, CHARFORMAT2& cf, PARAFORMAT2& pf)
{
	bool bSuc = false;
	if (NULL == lpstrText) return bSuc;
	long lStart = 0;
	long lEnd = 0;
	lStart = GetTextLength();
	lEnd = lStart;
	SetSel(lStart, lEnd);
	ReplaceSel(lpstrText, false);
	lEnd = GetTextLength();
	SetSel(lStart, lEnd);
	bSuc = SetSelectionCharFormat(cf);
	if (false == bSuc) return bSuc;
	bSuc = SetParaFormat(pf);
	SetSel(lEnd, lEnd);
	return bSuc;
}
...
int CRichEditUI::GetFirstVisibleLine() const {
	LRESULT lResult;
	TxSendMessage(EM_GETFIRSTVISIBLELINE, NULL, NULL, &lResult);
	return (int)lResult;
}
...
void CRichEditUI::UpdateView()
{
	if (NULL != m_pTwh)
	{
		m_pTwh->GetTextServices()->OnTxPropertyBitsChange(TXTBIT_VIEWINSETCHANGE, TXTBIT_VIEWINSETCHANGE);
	}
}

bool CRichEditUI::GetVScroll(LONG& min, LONG& max, LONG& pos, LONG& page, BOOL& enabled)
{
	bool bRet = false;
	if (NULL != m_pTwh)
	{
		m_pTwh->GetTextServices()->TxGetVScroll(&min, &max, &pos, &page, &enabled);
		bRet = true;
	}
...
int  CRichEditUI::GetVScrollThumbPos()
{
	RECT rect = GetPos();
	auto pt = PosFromChar(0);
	int offset = -pt.y;

	auto thumbPos = rect.top + offset;
	return thumbPos;
}

void CRichEditUI::SetVScrollThumbPos(int pos)
{
	WPARAM wParam = MAKEWPARAM(SB_THUMBPOSITION, pos);
	TxSendMessage(WM_VSCROLL, wParam, 0L, NULL);
}
...
{
	if (EN_CHANGE == iNotify)
		this->GetManager()->SendNotify(this, DUI_MSGTYPE_TEXTCHANGED);
	else if (EN_SELCHANGE == iNotify)
		this->GetManager()->SendNotify(this, DUI_MSGTYPE_SELCHANGED);
	switch(iNotify)
	{ 
	case EN_DROPFILES:   
	case EN_MSGFILTER:   
...
{
	//原来这种写法有BUG：当滑块滚到richedit接近底部，鼠标点击滚动条底部，触发PageDown时，最终会出现Richedit的最后一行内容跑到上面，下面出现一大片空白 hunter
	//TxSendMessage(WM_VSCROLL, SB_PAGEDOWN, 0L, 0);  

	__super::PageDown(); //调用基类方法可以解决此问题
}
...
void CRichEditUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_KILLFOCUS )  {
        if( m_pTwh ) {
			//richedit失去焦点时，这里会导致richedit内部刷新
			//最终导致：光标在图片行底，但滚动条不在richedit底部，窗口最小化再恢复，滚动条跑到图片顶部(可见行顶部对齐）
            //m_pTwh->OnTxInPlaceActivate(NULL); 
...
void CRichEditUI::OnPosChangeBegin()
{
}

void CRichEditUI::OnPosChangeEnd()
{
}
...
LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
	...
	if (uMsg == WM_IME_COMPOSITION) {
		...
		if (hImc) {
			...
			Composition.dwStyle = CFS_CANDIDATEPOS; //只有CFS_CANDIDATEPOS、CFS_EXCLUDE(符合预期); 
