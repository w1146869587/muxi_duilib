...
namespace DuiLib
{
	...
	void CTreeNodeUI::SetNodeIndex(CTreeNodeUI* _pTreeNodeUI, int index) 
	{
		mTreeNodes.SetAt(index, _pTreeNodeUI);
	}
...
// 			return -1;
		...
		if(_TreeNode)
		{
			if(_TreeNode->GetCountChild() > 0)
			{
				...
				for(int nIndex = 0;nIndex < nCount;nIndex++)
				{
					...
					{
						auto isExpanded = pItem->IsExpanded() && pItem->IsVisible();
						SetItemExpand(isExpanded, pItem);
					}
				}
			}
		}
	...
	// 函数说明: 
	//************************************
	void CTreeViewUI::SetItemMinWidth( int _ItemMinWidth )
	{
		CScale::GetInstance()->XScale(&_ItemMinWidth);
		m_uItemMinWidth = (UINT)_ItemMinWidth;
		for(int nIndex = 0;nIndex < GetCount();nIndex++){
