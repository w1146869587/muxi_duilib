...
void CActiveXUI::PixelToHiMetric(const LPSIZEL lpSizeInPix, LPSIZEL lpSizeInHiMetric)
{
    static int nPixelsPerInchX = 0;    // Pixels per logical inch along width
    static int nPixelsPerInchY = 0;    // Pixels per logical inch along height
	if (nPixelsPerInchX == 0 && nPixelsPerInchY == 0)
	{
...
void CActiveXUI::HimetricToPixel(LPSIZEL lpSizeHiMetric, LPSIZEL lpSizelPixels)
{
	static int cxPPI = 0;
	static int cyPPI = 0;
	...
	if (cxPPI == 0 && cyPPI == 0)
	{
		...
		cxPPI = GetDeviceCaps(hDCScreen, LOGPIXELSX);
		cyPPI = GetDeviceCaps(hDCScreen, LOGPIXELSY);

		::ReleaseDC(NULL, hDCScreen);
	}

	lpSizelPixels->cx = (long)MAP_LOGHIM_TO_PIX((int)lpSizeHiMetric->cx, cxPPI);
	lpSizeHiMetric->cy = (long)MAP_LOGHIM_TO_PIX((int)lpSizeHiMetric->cy, cyPPI);
}
