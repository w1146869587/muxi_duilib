...
namespace DuiLib
{
	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
		{
			if( ::PtInRect(&m_rcItem, event.ptMouse) && IsEnabled() ) {
				...
				Invalidate();
				if (NULL != m_pManager) m_pManager->SendNotify(this, DUI_MSGTYPE_BUTTONDOWN);
			}
			return;
		}
		if (event.Type == UIEVENT_RBUTTONDOWN)
		{
			if (::PtInRect(&m_rcItem, event.ptMouse) && IsEnabled()) {
				//m_uButtonState |= UISTATE_PUSHED | UISTATE_CAPTURED;
				//Invalidate();
				if (NULL != m_pManager) m_pManager->SendNotify(this, DUI_MSGTYPE_RBUTTONDOWN);
			}
			return;
		}
		...
		if( event.Type == UIEVENT_BUTTONUP )
		{
			if( (m_uButtonState & UISTATE_CAPTURED) != 0 )
			{
				m_uButtonState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED | UISTATE_HOT); //增加UISTATE_HOT为了解决长按小最化按钮，恢复窗口之后，最小化按钮还是hover状态 vito
				Invalidate();
	...
	void CButtonUI::SetPushedTextPadding(LPCTSTR pstrValue)
	{
		SetAttribute(_T("textpadding"), pstrValue);
		Invalidate();
	}
