...
CListUI::~CListUI() 
{
	//hunter，如果设置了List中的元素不自动清除，这里需要把m_pList和p_Header清除
	if (!m_bAutoDestroy)
	{
		if (m_pList)
		{
...
void CListUI::SetAutoDestroy(bool bAuto) 
{
	if (bAuto == false)
	{
		...
		m_pHeader->SetAutoDestroy(false);
	}
	else
	{
		m_pList->SetAutoDestroy(true);
		m_pHeader->SetAutoDestroy(true);
	}
...
{
	CScale::GetInstance()->RectScale(&rc);
...
{
    ASSERT(m_pOwner);
	//DUI__Trace(_T("CListBodyUI::CListBodyUI:%x\n"), this);
}

CListBodyUI::~CListBodyUI() {
	//DUI__Trace(_T("CListBodyUI::~CListBodyUI:%x\n"), this);
}
...
{
	m_iSepWidth = CScale::GetInstance()->XScale(iWidth);
}
...
void CListContainerElementUI::SetHover(bool bHover){
	if (!bHover){
		m_uButtonState = 0;
	}
}
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
    ...
    {
        if( IsEnabled() ){
            //m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMCLICK); //默认这种交互，鼠标快速双击组织架构展开部门显示有问题 hunter
            Select();
            Invalidate();
			m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMBTNDOWN, event.Type);//改为这里只激发一个自定义消息出去，等LBUTTONUP了再激发ITEMCLICK事件
        }
        return;
    }
    if( event.Type == UIEVENT_BUTTONUP ) 
    {
		if (IsEnabled())
		{
			m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMCLICK, event.Type);
		}
        return;
    }
    if( event.Type == UIEVENT_MOUSEMOVE )
    ...
    if( event.Type == UIEVENT_MOUSEENTER )
    {
        if( IsEnabled() ) {
            m_uButtonState |= UISTATE_HOT;
			m_pManager->SendNotify(this, DUI_MSGTYPE_MOUSEENTER, event.Type);
            Invalidate();
        }
        return;
    }
    if( event.Type == UIEVENT_MOUSELEAVE )
    {
        if( (m_uButtonState & UISTATE_HOT) != 0 ) {
            m_uButtonState &= ~UISTATE_HOT;
            Invalidate();
			m_pManager->SendNotify(this, DUI_MSGTYPE_MOUSELEAVE, event.Type);
        }
        return;
    }
