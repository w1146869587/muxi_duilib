...
void CControlUI::SetBkImage( HICON hIcon )
{
	if(NULL != m_hIcon)
	{
		DestroyIcon(m_hIcon);
...
{
	CScale::GetInstance()->XScale(&nSize);
...
{
	m_rcBorderSize = rc;
	CScale::GetInstance()->RectScale(&m_rcBorderSize);
	Invalidate();
}
...
bool CControlUI::DrawImage(HDC hDC, TDrawInfo& drawInfo)
{
	...
	RECT rcItem; //hunter:滚动条滑块不能拉伸填充满整个滚动条，滚动滑块大小放在drawInfo.rcDestOffset
	if (drawInfo.rcDestOffset.left != 0 || drawInfo.rcDestOffset.top != 0 ||
		drawInfo.rcDestOffset.right != 0 || drawInfo.rcDestOffset.bottom != 0) {
		rcItem.left = m_rcItem.left + drawInfo.rcDestOffset.left;
		rcItem.top = m_rcItem.top + drawInfo.rcDestOffset.top;
		rcItem.right = m_rcItem.left + drawInfo.rcDestOffset.right;
		
		if (rcItem.right > m_rcItem.right) {
			rcItem.right = m_rcItem.right;
		}
		
		rcItem.bottom = m_rcItem.top + drawInfo.rcDestOffset.bottom;
		
		if (rcItem.bottom > m_rcItem.bottom) {
...
// 	if (hDC == nullptr && m_pManager) { 打开会导致组织架构的人员头像覆盖到最近会话上面
// 		hDC = m_pManager->GetPaintDC();	
// 	}
	
	return CRenderEngine::DrawImageString(hDC, m_pManager, rcItem, m_rcPaint, drawInfo.sDrawString, drawInfo.sImageName);
}
...
void CControlUI::SetPadding(RECT rcPadding)
{
	...
	CScale::GetInstance()->RectScale(&m_rcPadding);
    NeedParentUpdate();
}
...
void CControlUI::SetFixedXY(SIZE szXY)
{
    m_cXY.cx = szXY.cx;
    m_cXY.cy = szXY.cy;
	CScale::GetInstance()->SizeScale(&m_cXY);
	NeedParentUpdate();
...
{
    if( cx < 0 ) return; 
	m_cxyFixed.cx = CScale::GetInstance()->XScale(cx);
	NeedParentUpdate();
...
void CControlUI::SetFixedWidthNoScale(int cx)
{
	if (cx < 0) return;
	m_cxyFixed.cx = cx;
	NeedParentUpdate();
...
{
    if( cy < 0 ) return; 
	m_cxyFixed.cy = CScale::GetInstance()->XScale(cy);
	NeedParentUpdate();
...
void CControlUI::SetFixedHeightNoScale(int cy)
{
	if (cy < 0) return;
	m_cxyFixed.cy = cy;
	NeedParentUpdate();
...
void CControlUI::SetFocusForce() {
	if (m_pManager != NULL) m_pManager->SetFocus(this, true);
}
...
void CControlUI::SetOwnerDraw(bool bOwnerDraw)
{
	m_bOwnerDraw = bOwnerDraw;
}

bool CControlUI::IsOwnerDraw()
{
	return m_bOwnerDraw;
}
...
void CControlUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	...
	if (IsOwnerDraw())
	{
		DUI_CONTROL_PAINT_LPARAM param;
		param.hdc = hDC;
		param.bHandle = false;
		param.paintRect = rcPaint;
		SendMessage(GetManager()->GetPaintWindow(), WM_DUI_CONTROL_BENGIN_PAINT, (WPARAM)GetName().GetData(), (LPARAM)&param);
		if (param.bHandle){	//已经处理过了就不用再画了
			PaintText(hDC);
			return;
		}
	}
...
{
	m_rcBorderSize.left = CScale::GetInstance()->XScale(nSize);
	Invalidate();
}
...
{
	m_rcBorderSize.top = CScale::GetInstance()->XScale(nSize);
	Invalidate();
}
...
{
	m_rcBorderSize.bottom = CScale::GetInstance()->XScale(nSize);
	Invalidate();
}
