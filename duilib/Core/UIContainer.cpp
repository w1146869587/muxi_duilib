...
namespace DuiLib
{
	...
	CContainerUI::~CContainerUI()
	{
		//DUI__Trace(_T("CContainerUI::~CContainerUI:%x\n"), this);
	...
	{
		CScale::GetInstance()->RectScale(&rcInset);
		m_rcInset = rcInset;
		NeedUpdate();
	}
	...
	{
		m_iChildPadding = CScale::GetInstance()->XScale(iPadding);
		NeedUpdate();
	}
	...
	{
		int cyLine = CScale::GetInstance()->XScale(8);
		if (m_pManager) cyLine = m_pManager->GetDefaultFontInfo()->tm.tmHeight + CScale::GetInstance()->XScale(8);
	...
	int CContainerUI::FindSelectable(int iIndex, bool bForward /*= true*/) const
	{
		...
		if( bForward ) {
			...
			return FindSelectable(0, true); //TODO: 0可以改为iIndex
		}
	}
