...
std::map<UINT, std::pair<HICON, HICON>> CWindowWnd::m_mapIcon;
...
UINT CWindowWnd::ShowModal(HWND hWndParent,bool bCenterWnd)
{
    ASSERT(::IsWindow(m_hWnd));
    UINT nRet = 0;
	...
	if (msg.message == WM_QUIT) {
		...
		::PostMessage(m_hWnd, WM_CLOSE, msg.wParam, msg.lParam);
	}
...
{
	HICON hIconMini = nullptr;
	HICON hIconMax = nullptr;
	if (m_mapIcon.find(nRes) != m_mapIcon.end()) {
		hIconMini = m_mapIcon[nRes].first;
		hIconMax = m_mapIcon[nRes].second;	
	} else {
		hIconMini = (HICON)::LoadImage(CPaintManagerUI::GetInstance(), MAKEINTRESOURCE(nRes), IMAGE_ICON, ::GetSystemMetrics(SM_CXICON), ::GetSystemMetrics(SM_CYICON), LR_DEFAULTCOLOR);
		hIconMax = (HICON)::LoadImage(CPaintManagerUI::GetInstance(), MAKEINTRESOURCE(nRes), IMAGE_ICON, ::GetSystemMetrics(SM_CXSMICON), ::GetSystemMetrics(SM_CYSMICON), LR_DEFAULTCOLOR);
		m_mapIcon[nRes] = { hIconMini, hIconMax };
	}

	::SendMessage(m_hWnd, WM_SETICON, (WPARAM)TRUE, (LPARAM)hIconMini);
	::SendMessage(m_hWnd, WM_SETICON, (WPARAM)FALSE, (LPARAM)hIconMax);
}
