...
const CDuiString& CPaintManagerUI::GetSkinFolder() {
	return m_pStrFolder;
}
...
void CPaintManagerUI::SetSkinFolder(LPCTSTR pStrPath) {
	m_pStrFolder = pStrPath;
	if (m_pStrFolder.IsEmpty()) return;
	TCHAR cEnd = m_pStrFolder.GetAt(m_pStrFolder.GetLength() - 1);
	if (cEnd != _T('\\') && cEnd != _T('/')) m_pStrFolder += _T('\\');
}
...
{
	m_szInitWindowSize.cx = CScale::GetInstance()->XScale(cx);
	m_szInitWindowSize.cy = CScale::GetInstance()->XScale(cy);
...
{
	CScale::GetInstance()->RectScale(&rcSizeBox);
...
{
	CScale::GetInstance()->RectScale(&rcCaption);
...
void CPaintManagerUI::SetMinInfo(int cx, int cy)
{
    ...
    m_szMinWindow.cx = cx;
    m_szMinWindow.cy = cy;
	CScale::GetInstance()->SizeScale(&m_szMinWindow);
}

void CPaintManagerUI::SetMinInfoWithNoScale(int cx, int cy)
{
	ASSERT(cx >= 0 && cy >= 0);
	m_szMinWindow.cx = cx;
	m_szMinWindow.cy = cy;
...
void CPaintManagerUI::SetMaxInfo(int cx, int cy)
{
    ...
    m_szMaxWindow.cx = cx;
    m_szMaxWindow.cy = cy;
	CScale::GetInstance()->SizeScale(&m_szMaxWindow);
}
...
{
	bool bSetColorKey = false;
	if ((nOpacity & 0xFF000000) == 0xFF000000)
	{
		bSetColorKey = true;
	}
	else if (nOpacity<0)
    ...
    if( m_hWndPaint != NULL ) {
		...
		if (bSetColorKey) {
			fSetLayeredWindowAttributes(m_hWndPaint, nOpacity & 0x00FFFFFF, 128, LWA_COLORKEY);
		} else {
...
void CPaintManagerUI::SetDrawNeeded()
{
	m_bDrawNeeded = true;
}
    ...
    case WM_KEYDOWN:
        {
           ...
           if( wParam == VK_TAB ) {
               if( m_pFocus && m_pFocus->IsVisible() && m_pFocus->IsEnabled()) {
				   ...
				   else if (_tcsstr(m_pFocus->GetClass(), _T("WkeWindowlessUI")) != NULL) {
					   if (static_cast<CWkeWindowlessUI*>(m_pFocus)->IsWantTab()) return false;
				   }
			   }
...
//#endif
    ...
    case WM_PAINT:
        {
		   ...
		   if(m_bAlphaBackground)
		   {
			   ...
			   if ((m_bOffscreenPaint && m_hbmpOffscreen == NULL) || m_paintWidth != nClientWidth || m_paintHeight != nClientHeight)	//窗口大小改变要重新获取位图数据, 不然改变大小的时候会崩掉 seven
			   {
				   m_paintWidth = nClientWidth;
				   m_paintHeight = nClientHeight;
	   ...
	   if(m_bUpdateNeeded)  {
		...
		if (m_bDrawNeeded) {
			m_bDrawNeeded = false;
			SendNotify(m_pRoot, DUI_MSGTYPE_DRAW, 0, 0, false);
		}
	...
	case WM_IME_STARTCOMPOSITION:      //输入法
		...
		break;
// 	case WM_RBUTTONUP:                 //这是给webkit增加一个RButtoUP事件，但是该事件会影响菜单弹出，先注释掉 hunter
// 		{
// 			POINT pt = { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
// 			m_ptLastMousePos = pt;
// 			if (m_pEventClick == NULL) break;
...
void CPaintManagerUI::ReapObjects(CControlUI* pControl)
{
    ...
    if (pControl == m_pEventClick)  {
			ReleaseCapture(); //hunter.lin 解决双击最近会话弹出IM窗口，IM窗口不能点击，最近会话双击时（第三个动作）会去清除会话列表，因此这里需要ReleaseCapture。
			m_pEventClick = NULL;
	}
...
void CPaintManagerUI::SetFocus(CControlUI* pControl, bool isForce)
{
    ...
    // Already has focus?
	if (pControl == m_pFocus && isForce == false) return;
    // Remove focus from old control
    if( m_pFocus != NULL ) 
    {
		//DUITRACE(_T("KillFocus CtrlName:%s"), m_pFocus->GetName().GetData());
        TEventUI event = { 0 };
    ...
    if( pControl != NULL 
        ...
        && pControl->IsEnabled() ) 
    {
		//DUITRACE(_T("GetFocus CtrlName:%s"), pControl->GetName().GetData());
        m_pFocus = pControl;
        TEventUI event = { 0 };
...
void CPaintManagerUI::SetOwnerDraw(bool bOwnerDraw){
	m_isOwnerDraw = bOwnerDraw;
}

bool CPaintManagerUI::IsOwnerDraw(){
	return m_isOwnerDraw;
}
...
void CPaintManagerUI::SetDefaultFont(LPCTSTR pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic, bool bShared)
{
    ...
    lf.lfCharSet = DEFAULT_CHARSET;
	lf.lfHeight = -CScale::GetInstance()->XScale(nSize);
	...
	if (bShared)
	{
		...
		m_SharedResInfo.m_DefaultFontInfo.iSize = CScale::GetInstance()->XScale(nSize);
