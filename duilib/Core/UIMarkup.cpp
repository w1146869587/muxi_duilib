...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, CPaintManagerUI* pManager, int encoding)
{
	...
	if (pManager != nullptr)
	{
		sFolder = pManager->GetSkinFolder();
	}
     
    if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
		auto rootPath = CPaintManagerUI::GetResourcePath();
		auto tmpFileName = rootPath + sFolder + pstrFilename;
		HANDLE hFile = ::CreateFile(tmpFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		CDuiString tmpFileName = sFolder +  pstrFilename;
        CDuiString sZipFile = sFolder + CPaintManagerUI::GetResourceZip();
        HZIP hz = NULL;
        ...
        ZIPENTRY ze; 
        int i; 
		if (FindZipItem(hz, tmpFileName, true, &i, &ze) != 0) return _Failed(_T("Could not find ziped file"));
