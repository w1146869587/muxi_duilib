...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, CPaintManagerUI* pManager, LPCTSTR type, DWORD mask)
{
	...
	{
		if( type == NULL ) {
			CDuiString sFolder = pManager->GetSkinFolder();
			if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
				auto sFullFileName = CPaintManagerUI::GetResourcePath() + sFolder + bitmap.m_lpstr;
				HANDLE hFile = ::CreateFile(sFullFileName.GetData(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, \
					FILE_ATTRIBUTE_NORMAL, NULL);
				...
				auto sFullFileName = _getRealPath(pManager, bitmap.m_lpstr);
				auto sZipFile = sFolder + CPaintManagerUI::GetResourceZip();
				HZIP hz = NULL;
				if( CPaintManagerUI::IsCachedResourceZip() ) hz = (HZIP)CPaintManagerUI::GetResourceZipHandle();
				else hz = OpenZip((void*)sZipFile.GetData(), 0, 2);
				if( hz == NULL ) break;
				ZIPENTRY ze; 
				int i; 
				if (FindZipItem(hz, sFullFileName, true, &i, &ze) != 0) break;
				dwSize = ze.unc_size;
				if( dwSize == 0 ) break;
				pData = new BYTE[ dwSize ];
				int res = UnzipItem(hz, i, pData, dwSize, 3);
	...
	if (!pData)
	{
		DUI__Trace(_T("Load image failed, bitmap:%s"), bitmap);
		//::MessageBox(0, _T("读取图片数据失败！"), _T("抓BUG"), MB_OK);
		return NULL;
	}
...
HICON  CRenderEngine::LoadIcon(STRINGorID bitmap) {
	HICON hIcon = nullptr;
	if (CPaintManagerUI::GetResourceZip().IsEmpty()) {
		CDuiString path = CPaintManagerUI::GetResourcePath();
		path += bitmap.m_lpstr;
		hIcon = (HICON)::LoadImage(NULL, path, IMAGE_ICON, 16, 16, LR_LOADFROMFILE);    //原函数默认写入参数为 0*0, 显示模糊, 改为16*16
	} else  {
		auto sFullFileName = bitmap.m_lpstr;
		auto sFile = CPaintManagerUI::GetResourceZip();
		HZIP hz = NULL;
		if (CPaintManagerUI::IsCachedResourceZip()) hz = (HZIP)CPaintManagerUI::GetResourceZipHandle();
		else hz = OpenZip((void*)sFile.GetData(), 0, 2);
		if (hz == NULL) return nullptr;
		ZIPENTRY ze;
		int i;
		if (FindZipItem(hz, sFullFileName, true, &i, &ze) != 0) return nullptr;
		auto dwSize = ze.unc_size;
		...
		int offset = ::LookupIconIdFromDirectoryEx(pData, TRUE, 16, 16, LR_DEFAULTCOLOR);
		if (offset != 0) {
			hIcon = ::CreateIconFromResourceEx(pData + offset, 0, TRUE, 0x30000, 16, 16, LR_DEFAULTCOLOR);
		}
	...
	bool bHole, bool bTiledX, bool bTiledY)
{
	...
	//::DeleteObject(data->hBitmap);//eason.gao: 这里为什么DeleteObject按钮显示不正常
...
{
	if ((pManager == NULL) || (hDC == NULL) || pStrImage == NULL || _tcslen(pStrImage) == 0) return false;
			...
			if (!sValue.IsEmpty()) {
				if (sItem == _T("file") || sItem == _T("res")) {
					if (image_count > 0) {
						if (true == bDest) {
							if (true == bScale) {
								CScale::GetInstance()->RectScale(&rcDest);
							}

							rcItem.left = rc.left + rcDest.left;
							rcItem.top = rc.top + rcDest.top;
							rcItem.right = rc.left + rcDest.right;
							if (rcItem.right > rc.right) {
								rcItem.right = rc.right;
							}

							rcItem.bottom = rc.top + rcDest.bottom;
							{
				...
				else if (sItem == _T("scale")) {
					bScale = (_tcscmp(sValue.GetData(), _T("true")) == 0);
				}
			}
			if (*pStrImage++ != _T(' ')) break;
		}
...
CDuiString CRenderEngine::_getRealPath(CPaintManagerUI* pManager, const CDuiString& sFilePath) { //hunter:将相对路径转换为绝对路径，主要用于cache zip查找对象
	if (pManager == nullptr) {
		return sFilePath;
	}

	//最终结果：Folder + FilePath 并转换掉../的相对路径

	CDuiString sFullFilePath = pManager->GetSkinFolder() + sFilePath;
	auto nPos1 = sFilePath.Find(_T(".."));
	if (nPos1 != -1) {
		sFullFilePath.Replace(_T("\\"), _T("/"));
		sFullFilePath.Replace(_T("//"), _T("/"));

		std::vector<CDuiString> vectSplit;
		int nStart = 0;
		while (true) {
			int nEnd = sFullFilePath.Find(_T("/"), nStart);
			if (nEnd == -1) {
				vectSplit.push_back(sFullFilePath.Mid(nStart, sFullFilePath.GetLength() - nStart));
				break;
			}

			vectSplit.push_back(sFullFilePath.Mid(nStart, nEnd - nStart));
			nStart = nEnd + _tcslen(_T("/"));
		}

		std::vector<CDuiString> vectDirName;
		for (int i = 0; i < int(vectSplit.size()); i++) {
			if (vectSplit[i] != _T("..")) {
				vectDirName.push_back(vectSplit[i]);
			} else {
				if (!vectDirName.empty()) {
					vectDirName.pop_back();
				}
			}
		}

		CDuiString strNewPath = _T("");
		for (int i = 0; i < int(vectDirName.size()); i++) {
			if (vectDirName.size() - 1 == i) {
				strNewPath += vectDirName[i];
			} else {
				strNewPath += vectDirName[i] + _T("/");
			}
		}

		sFullFilePath = strNewPath;
	}
