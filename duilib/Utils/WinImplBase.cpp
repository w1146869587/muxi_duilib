...
namespace DuiLib
{
#define  IMG_WINDOW_BORDER		_T("../common/100a_030.png")
//////////////////////////////////////////////////////////////////////////
...
DUI_BEGIN_MESSAGE_MAP(WindowImplBase,CNotifyPump)
	...
	DUI_ON_MSGTYPE(DUI_MSGTYPE_DRAW, OnDraw)
DUI_END_MESSAGE_MAP()

WindowImplBase::WindowImplBase()
{
	m_isShowShadowWin = true;
}
...
LRESULT WindowImplBase::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	{
        CControlUI* pbtnMax = static_cast<CControlUI*>(m_PaintManager.FindControl(_T("max_btn")));         // max button
        CControlUI* pbtnRestore = static_cast<CControlUI*>(m_PaintManager.FindControl(_T("restore_btn"))); // restore button
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	//}
		m_PaintManager.SetSkinFolder(GetSkinFolder().GetData());
	...
	{
// 		auto folder = GetSkinFolder();
// 		auto skinFile = folder + _T("/") +  GetSkinFile();
...
LRESULT WindowImplBase::OnNoActivateCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
// 	LONG styleValue = ::GetWindowLong(*this, GWL_STYLE);
// 	styleValue &= ~WS_CAPTION;
// 	::SetWindowLong(*this, GWL_STYLE, styleValue | WS_CLIPSIBLINGS | WS_CLIPCHILDREN);
// 	RECT rcClient;
...
{
	//CDuiString strLog;
	//strLog.Format(_T("uMsg:%d, wParam:%d, lParam:%d\n"), uMsg, wParam, lParam);
...
void WindowImplBase::OnClick(TNotifyUI& msg)
{
	...
	if( sCtrlName == _T("close_btn") )
	{
		Close();
		return; 
	}
	else if( sCtrlName == _T("mini_btn"))
	{ 
		SendMessage(WM_SYSCOMMAND, SC_MINIMIZE, 0); 
		return; 
	}
	else if( sCtrlName == _T("max_btn"))
	{ 
		SendMessage(WM_SYSCOMMAND, SC_MAXIMIZE, 0); 
		return; 
	}
	else if( sCtrlName == _T("restore_btn"))
	{ 
...
void WindowImplBase::OnDraw(TNotifyUI& msg) {
	if (m_isShowShadowWin) {
		CreateShadowWnd();
	}
}

LRESULT WindowImplBase::OnShowWindow(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	if (nullptr != m_pShadowWin) {
		BOOL isShow = (BOOL)wParam;
		if (false == isShow) {
			m_PaintManager.SetDrawNeeded();
			m_pShadowWin->ShowWindow(false, false);
		}
	}
...
void WindowImplBase::ShowShadowWnd(bool isShow) {
	if (nullptr != m_pShadowWin) {
		m_pShadowWin->ShowWindow(isShow, false);
	}
}

void WindowImplBase::CreateShadowWnd() {
	if (nullptr == m_pShadowWin) {
		m_pShadowWin = new CUIShadowWnd(IMG_WINDOW_BORDER);

		auto bRet = m_pShadowWin->Init(&m_PaintManager);
		if (!bRet) {
			delete m_pShadowWin;
			m_pShadowWin = nullptr;
		}
	}

	if (m_pShadowWin != nullptr)
	{
		m_pShadowWin->SetWndPos();
		m_pShadowWin->ShowWindow(true, false);
	}
}

void WindowImplBase::NotShowShadowWnd() {
	m_isShowShadowWin = false;
}
}